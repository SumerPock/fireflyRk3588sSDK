...
#define IMX586_LINK_FREQ_1250		1250000000

#define IMX586_LANES			4

#define PIXEL_RATE_WITH_848M_10BIT	(IMX586_LINK_FREQ_1250 / 10 * 4 * 2)
...
static const struct regval imx586_linear_10bit_2000x1128_global_regs[] = {
	{0x0136, 0x18},
	{0x0137, 0x00},
	{0x3C7E, 0x02},
	{0x3C7F, 0x0A},
...
static const struct regval imx586_linear_2000x1128_240fps_regs[] = {
	{0x0112, 0x0A},
	{0x0113, 0x0A},
	{0x0114, 0x03},
	{0x0342, 0x15},
...
static const struct imx586_mode supported_modes[] = {
	{		
		...
		.global_reg_list = imx586_linear_10bit_2000x1128_global_regs,
		.reg_list = imx586_linear_2000x1128_240fps_regs,
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 2,
...
/*	
	{	
		...
		.bus_fmt = MEDIA_BUS_FMT_SRGGB10_1X10,
		.global_reg_list = imx586_linear_10bit_2000x1128_global_regs,
		.reg_list = imx586_linear_2000x1128_120fps_regs,
		.hdr_mode = NO_HDR,
		.mipi_freq_idx = 2,
		.vc[PAD0] = V4L2_MBUS_CSI2_CHANNEL_0,
	},	
				...
				struct v4l2_subdev_selection *sel)
{
	...
	if (sel->target == V4L2_SEL_TGT_CROP_BOUNDS) {
		if (imx586->cur_mode->height == 1128) {
			sel->r.left = CROP_START(imx586->cur_mode->width, DST_WIDTH_1920);
			sel->r.width = DST_WIDTH_1920;
			sel->r.top = CROP_START(imx586->cur_mode->height, DST_HEIGHT_1080);
			sel->r.height = DST_HEIGHT_1080;
		} else {
			sel->r.left = CROP_START(imx586->cur_mode->width, imx586->cur_mode->width);
			sel->r.width = imx586->cur_mode->width;
			sel->r.top = CROP_START(imx586->cur_mode->height, imx586->cur_mode->height);
			sel->r.height = imx586->cur_mode->height;
		}
		return 0;
	}
	return -EINVAL;
...
static int imx586_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		max = imx586->cur_mode->height + ctrl->val - 48;
	...
	case V4L2_CID_ANALOGUE_GAIN:
		...
		break;
	case V4L2_CID_VBLANK:
		vts = ctrl->val + imx586->cur_mode->height;	
		ret = imx586_write_reg(imx586->client,
				       IMX586_REG_VTS_H,
				       IMX586_REG_VALUE_08BIT,
				       vts >> 8);
